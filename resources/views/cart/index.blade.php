...
<div class="container mx-auto px-4 py-8 md:py-10 pb-32 sm:pb-12">
    @if($carts->isEmpty())
        <div class="text-center py-20 border border-dashed border-gray-200 rounded-xl">
            ...
            <a href="{{ route('home') }}" class="inline-block bg-dark text-white px-8 py-3 font-bold uppercase text-[10px] tracking-widest hover:bg-primary transition-all">
                Mulai Belanja
            </a>
        </div>
    @else
                        ...
                        <div class="flex items-center gap-4 sm:gap-6 py-6 border-b border-gray-100 group">
                            ...
                            <div class="w-20 h-20 sm:w-28 sm:h-28 flex-shrink-0 bg-gray-50 border border-gray-100 rounded p-2 overflow-hidden relative">
                                @if($cart->product->image)
                                    <img src="{{ asset('storage/' . $cart->product->image) }}" alt="{{ $cart->product->name }}" class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500">
                                @else
...
<div class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 lg:hidden z-50 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
    <div class="max-w-xl mx-auto">
        <div class="flex justify-between items-center mb-3">
            ...
            <button onclick="document.getElementById('mobile-drawer').classList.add('translate-y-0')" class="text-[9px] font-bold uppercase border-b border-dark text-dark">
        ...
        <div id="mobile-drawer" class="fixed inset-0 bg-white z-[60] transform translate-y-full transition-transform duration-500 p-6 flex flex-col">
            <div class="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                ...
                <button type="button" onclick="document.getElementById('mobile-drawer').classList.remove('translate-y-0')" class="w-8 h-8 flex items-center justify-center bg-gray-50 rounded-full">
            ...
            <form action="{{ route('checkout.process') }}" method="POST" class="flex flex-col h-full">
                ...
                <div class="flex-1 space-y-6">
                    <div>
                        <label class="text-[9px] font-bold uppercase tracking-widest text-gray-400 block mb-2">Alamat Lengkap</label>
                        <textarea name="address" required class="w-full border border-gray-100 p-4 text-xs font-medium focus:outline-none focus:border-dark h-28" placeholder="Nama jalan, nomor rumah..."></textarea>
                    </div>
                    <div>
                        <label class="text-[9px] font-bold uppercase tracking-widest text-gray-400 block mb-2">Catatan Pesanan</label>
                        <input type="text" name="note" class="w-full border border-gray-100 p-4 text-xs font-medium focus:outline-none focus:border-dark" placeholder="Contoh: Titip di pos satpam">
                    </div>
                </div>
        ...
        <button onclick="document.getElementById('mobile-drawer').classList.add('translate-y-0')" class="w-full bg-dark text-white py-3.5 font-bold uppercase tracking-widest text-[10px] hover:bg-primary active:scale-95 transition-all">
            Checkout Sekarang
        </button>
    </div>
</div>
