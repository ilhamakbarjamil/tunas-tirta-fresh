    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
        ...
        <div class="bg-white border border-gray-100 flex flex-col group transition-all duration-300 hover:shadow-xl relative">
            ...
            <div class="p-4 flex-1 flex flex-col">
                ...
                <div class="mt-auto">
                    ...
                    <p class="text-base font-black text-primary tracking-tighter" id="price-display-{{ $product->id }}">
                        ...
                        <span class="text-[9px] text-gray-400 font-bold ml-1 uppercase">/ {{ $product->unit }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="p-4 bg-white border-t border-gray-50">
                ...
                @else
                    <div class="pt-2">
                        <button type="button" disabled class="w-full bg-gray-100 text-gray-300 font-black py-3 text-[10px] uppercase tracking-widest rounded-none cursor-not-allowed border border-gray-200">
                            Stok Habis 
                        </button>
                    </div>
                @endif
    ...
    <div class="relative bg-white w-full max-w-5xl flex flex-col md:flex-row shadow-2xl rounded-none overflow-hidden border-none transform scale-95 transition-all duration-500" id="promo-content">
        ...
        <div class="w-full md:w-1/2 bg-gray-100 relative min-h-[250px] md:min-h-[500px]">
            @if($activePromo->image)
                <img src="{{ asset('storage/' . $activePromo->image) }}" class="absolute inset-0 w-full h-full object-cover" alt="{{ $activePromo->title }}">
                <div class="absolute top-4 left-4 bg-primary text-white font-black px-3 py-1 text-[8px] uppercase tracking-[0.2em]">Announcement</div>
            @endif
        </div>
        <div class="w-full md:w-1/2 p-8 md:p-14 flex flex-col justify-center bg-white">
            <div class="space-y-6">
                <div>
                    <span class="text-primary font-black tracking-[0.3em] uppercase text-[9px] mb-2 block">Official Info</span>
                    <h2 class="text-3xl md:text-4xl font-black text-dark leading-[0.9] uppercase tracking-tighter">{{ $activePromo->title }}</h2>
                </div>
                <div class="w-10 h-[2px] bg-primary"></div>
                <p class="text-gray-500 text-[12px] font-bold leading-relaxed uppercase tracking-tight text-justify">{{ $activePromo->description }}</p>
                <div class="pt-4 space-y-4">
                    @if($activePromo->button_link)
                        <a href="{{ url($activePromo->button_link) }}" class="block w-full bg-dark hover:bg-primary text-white text-center font-black py-4 text-[10px] uppercase tracking-[0.2em] transition-all group">
                            {{ $activePromo->button_text }} <i class="fas fa-arrow-right ml-2 text-[8px] transform group-hover:translate-x-1 transition-transform"></i>
                        </a>
                    @else
                        <button onclick="closePromo()" class="block w-full bg-dark hover:bg-primary text-white text-center font-black py-4 text-[10px] uppercase tracking-[0.2em] transition-all">{{ $activePromo->button_text }}</button>
                    @endif
                    <button onclick="closePromo()" class="block w-full text-center text-gray-300 text-[9px] font-black uppercase tracking-widest hover:text-dark transition-colors">Abaikan Pesan Ini</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    function updatePrice(selectElement, productId) {
        ...
        if (newPrice) {
            ...
            // KITA TETAP PERTAHANKAN SATUAN SAAT GANTI HARGA
            // Ambil satuan dari elemen span yang sudah ada (kalau mau dinamis harus via ajax, tapi ini static dulu aman)
